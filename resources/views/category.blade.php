                ...
                <div class="row">
                    ...
                    <div class="col-md-7">
                        <h5 class="showing-pagination-results mt-5 mt-md-9 text-center text-md-end">
                            {{ __('messages.Showing') }} {{ $category->products_count }} {{ __('messages.Results') }}</h5>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section class="section-space pb-0">
            <div class="container">
                ...
                <div class="row g-3 g-sm-6">
                    ...
                    @endphp
                    @foreach ($categories as $index => $productcategory)
                        <div class="col-6 col-lg-4 col-lg-2 col-xl-2 mt-xl-0 mt-sm-6 mt-4">
                            <!--== Start Product Category Item ==-->
                            <a href="{{ route('details_category', $productcategory->id) }}" class="product-category-item"
                                data-bg-color="{{ $colors[$index % count($colors)] }}">
                                <img class="icon" src="{{ URL::asset('storage/' . $productcategory->avatar) }}"
                                    width="80" height="80" alt="Image-HasTech">
                                <h3 class="title">
                                    {{ app()->getLocale() == 'ar' ? $productcategory->ar_details : $productcategory->details }}
                                </h3>
                            </a>
...
@if ($category->products != null)
    <script>
        function fetchSortedProducts() {
            ...
            var cat_id = @json($category->id);

            fetch(`/products/sort/${cat_id}/${sort}`, {
        ...
        function updateProductSection(products) {
            if (products !== null) {
                ...
                products.forEach(product => {
                    ...
                    <div class="product-item">
                        <div class="product-thumb">
                            <a class="d-block" href="/product/details/${product.id}">
                                <img src="/storage/${product.image}" width="370" style="height: 450px" alt="Image-HasTech">
                            </a>
                        </div>
                        <div class="product-info">
                            <h4 class="title"><a href="/product/details/${product.id}">${appLocale === 'ar' ? product.name : product.ar_name}</a></h4>
                            <div class="prices">
